...
namespace ola
{
	...
	Bool X86LoadBaseDispTile::Match(ISelNode* node)
	{
		matched_reg = nullptr;
		matched_load = nullptr;
		base = nullptr;
	...
	TileResult X86LoadBaseDispTile::Apply(MachineContext& ctx)
	{
		TileResult result;
		...
		load.SetOp<0>(matched_reg->GetRegister());
		load.SetOp<1>(base_op);
		load.SetOp<2>(MachineOperand::Undefined());
		load.SetOp<3>(MachineOperand::Immediate(1, MachineType::Int64));
		load.SetOp<4>(MachineOperand::Immediate(displacement, MachineType::Int64));
		result.instructions.push_back(load);
	...
	Bool X86LoadIndexScaleDispTile::Match(ISelNode* node)
	{
		matched_reg = nullptr;
		matched_load = nullptr;
		matched_mul = nullptr;
	...
	TileResult X86LoadIndexScaleDispTile::Apply(MachineContext& ctx)
	{
		TileResult result;
		...
		MachineInstruction load(InstLoad);
		load.SetOp<0>(matched_reg->GetRegister());
		load.SetOp<1>(MachineOperand::Undefined());
		load.SetOp<2>(index_op);
		load.SetOp<3>(MachineOperand::Immediate(scale, MachineType::Int64));
		load.SetOp<4>(MachineOperand::Immediate(displacement, MachineType::Int64));
		result.instructions.push_back(load);
	...
	Bool X86LoadBaseIndexScaleDispTile::Match(ISelNode* node)
	{
		matched_reg = nullptr;
		matched_load = nullptr;
		outer_add = nullptr;
		...
		auto* outer = dyn_cast<ISelBinaryOpNode>(load->GetAddress());
	...
	TileResult X86LoadBaseIndexScaleDispTile::Apply(MachineContext& ctx)
	{
		TileResult result;
	...
	Bool X86StoreBaseDispTile::Match(ISelNode* node)
	{
		matched_store = nullptr;
		base = nullptr;
		value = nullptr;
	...
	TileResult X86StoreBaseDispTile::Apply(MachineContext& ctx)
	{
		TileResult result;
		...
		MachineInstruction store(InstStore);
		store.SetOp<0>(base_op);
		store.SetOp<1>(MachineOperand::Undefined());
		store.SetOp<2>(MachineOperand::Immediate(1, MachineType::Int64));
		store.SetOp<3>(MachineOperand::Immediate(displacement, MachineType::Int64));
		store.SetOp<4>(value_op);
		result.instructions.push_back(store);
	...
	Bool X86StoreIndexScaleDispTile::Match(ISelNode* node)
	{
		matched_store = nullptr;
		matched_mul = nullptr;
		index = nullptr;
	...
	TileResult X86StoreIndexScaleDispTile::Apply(MachineContext& ctx)
	{
		TileResult result;
		...
		MachineInstruction store(InstStore);
		store.SetOp<0>(MachineOperand::Undefined());
		store.SetOp<1>(index_op);
		store.SetOp<2>(MachineOperand::Immediate(scale, MachineType::Int64));
		store.SetOp<3>(MachineOperand::Immediate(displacement, MachineType::Int64));
		store.SetOp<4>(value_op);
	...
	Bool X86StoreBaseIndexScaleDispTile::Match(ISelNode* node)
	{
		matched_store = nullptr;
		outer_add = nullptr;
		inner_add = nullptr;
		...
		auto* outer = dyn_cast<ISelBinaryOpNode>(store->GetAddress());
		if (!outer || outer->GetOpcode() != Opcode::Add) return false;
	...
	TileResult X86StoreBaseIndexScaleDispTile::Apply(MachineContext& ctx)
	{
		TileResult result;
	...
	{
		tiler.RegisterTile(std::make_unique<X86LoadBaseIndexScaleDispTile>());
		tiler.RegisterTile(std::make_unique<X86StoreBaseIndexScaleDispTile>());
		...
		tiler.RegisterTile(std::make_unique<X86LoadIndexScaleDispTile>());
		tiler.RegisterTile(std::make_unique<X86StoreIndexScaleDispTile>());
		tiler.RegisterTile(std::make_unique<X86LeaIndexScaleDispTile>());
		tiler.RegisterTile(std::make_unique<X86LoadBaseDispTile>());
		tiler.RegisterTile(std::make_unique<X86StoreBaseDispTile>());
