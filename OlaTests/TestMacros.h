...
#define OLA(...) system(OLA_STRINGIFY(OLA_EXE_PATH --Od --nollvm --test __VA_ARGS__))
#define OLA_SIMPLE(...) system(OLA_STRINGIFY(OLA_EXE_PATH --testdebug --test --nollvm --simple OLA_CODE(__VA_ARGS__)))
#define OLA_EX_SIMPLE(...) system(OLA_STRINGIFY(OLA_EXE_PATH --testdebug --test --nollvm --simple OLA_CODE_EX(__VA_ARGS__)))
#else 
#define OLA_EXE_PATH OLA_CONCAT(OLA_BINARY_PATH, "Release/Ola.exe")
#define OLA(...) system(OLA_STRINGIFY(OLA_EXE_PATH --O3 --nollvm --test __VA_ARGS__))
#define OLA_SIMPLE(...) system(OLA_STRINGIFY(OLA_EXE_PATH --test --nollvm --simple OLA_CODE(__VA_ARGS__)))
#define OLA_EX_SIMPLE(...) system(OLA_STRINGIFY(OLA_EXE_PATH --test --nollvm --simple OLA_CODE_EX(__VA_ARGS__)))
#endif
