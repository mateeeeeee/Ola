...
#ifdef WAVE_DEBUG_BUILD
#define WAVE_EXE_PATH WAVE_CONCAT(WAVE_BINARY_PATH, "Debug/Wave.exe")
#define WAVE(...) system(WAVE_STRINGIFY(WAVE_EXE_PATH --Od --test __VA_ARGS__))
#define WAVE_SIMPLE(...) system(WAVE_STRINGIFY(WAVE_EXE_PATH --testdebug --test --simple WAVE_CODE(__VA_ARGS__)))
#define WAVE_EX_SIMPLE(...) system(WAVE_STRINGIFY(WAVE_EXE_PATH --testdebug --test --simple WAVE_CODE_EX(__VA_ARGS__)))
#else 
